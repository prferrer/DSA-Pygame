...
def get_valid_powerup_spawn():
    ...
    while True:
        # Pick a random column index (X)
        x = random.randint(0, md.MAP_COLS-1)
        # Pick a random row index (Y)
        y = random.randint(0, md.MAP_ROWS-1)
        
        # Check the map grid; if the value is 0 (empty floor)
...
# Only process powerup timers if the game is actively running (not in the win screen)
    if game_state == "playing":
        ...
        # If the green powerup (Speed) doesn't currently exist on the map AND 
        # the current game time has surpassed its scheduled spawn time...
        if not green_powerup and current_time >= green_spawn_time:
            # Call the helper function to get an empty tile and assign it to the green powerup
            green_powerup = get_valid_powerup_spawn()
            # Set the time when this powerup should vanish if nobody picks it up (current time + 15 seconds)
            green_despawn_time = current_time + POWERUP_DESPAWN
            
        # Do the exact same spawn check for the blue powerup (Slowdown)
        if not blue_powerup and current_time >= blue_spawn_time:
            # Spawn it on a valid tile
            blue_powerup = get_valid_powerup_spawn()
            # Set its despawn timer
            blue_despawn_time = current_time + POWERUP_DESPAWN
            
        # If the green powerup is currently on the map AND the current time has surpassed its lifespan limit...
        if green_powerup and current_time >= green_despawn_time:
            # Remove it from the map by setting its coordinate variable to None
            green_powerup = None
            # Schedule its next appearance (current time + 10 seconds respawn delay)
            green_spawn_time = current_time + POWERUP_RESPAWN
            
        # Do the exact same despawn check for the blue powerup
        if blue_powerup and current_time >= blue_despawn_time:
            # Remove it from the map
            blue_powerup = None
            # Schedule its next appearance
